"""
Step 3: Two-tier verification using BlackboxAI (OpenAI-compatible API).
...
try:
    ...
    log.info("ERROR: openai package required. Install with: pip install openai")
    sys.exit(1)
...
MODEL = "moonshotai/kimi-vl-a3b-thinking:free"  # user-requested model for verification
MAX_RETRIES = 3
RETRY_DELAY = 5
...
def call_llm(client, prompt, content):
    """Call BlackboxAI via OpenAI-compatible API."""
...
def main():
    ...
    if not api_key:
        log.info("ERROR: BLACKBOX_API_KEY not found in environment or .env files.")
        log.info(f"Checked: {[str(p) for p in ENV_PATHS]}")
        sys.exit(1)
    ...
    for ext_path in extraction_files:
        ...
        result = call_llm(client, TIER1_PROMPT, json.dumps(extraction, indent=2))
        if result is None:
            errors += 1
            continue
        ...
        if needs_html:
            ...
            if html_path.exists():
                ...
                tier2_result = call_llm(client, TIER2_PROMPT, tier2_content)
                if tier2_result:
